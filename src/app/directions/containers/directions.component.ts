...
export class DirectionsComponent implements OnInit, OnDestroy {
	...
	async ngOnInit() {
		...
		this.venueSubscription = this.venueService.getVenueObservable()
			.subscribe((venue: Venue) => {
				if (venue.id) {
					this.venue = venue;
					this.populateSearchParams(venue.id);
				}
			});
		...
		const destinationPromise = this.populateDestination(this.route.snapshot.params.id ? this.route.snapshot.params.id : this.route.snapshot.params.to);
	...
	private populateOrigin() {
		return new Promise(async (resolve, reject) => {
			...
			if (locationId) {
				...
				this.originInputValue = 'My Position';
				this.currentPositionVisible = false;
	...
	handleMyPositionClick(position) {
		this.currentPositionVisible = false;
		this.originLocation = {
			properties: { name: position.name },
			geometry: { coordinates: [position.coordinates[0], position.coordinates[1]] }
		};
	 ...
	 * @returns {Promise}
	 */
	private populateDestination(id) {
	...
	switchOriginAndDest() {
		...
		this.clearRoute();
		if (this.originLocation && this.destinationLocation) this.getRoute();
		// Google Analytics
	...
	selectLocation(location) {
		...
		if (this.currentInputField === 'start') {
			...
			// Google Analytics
			ga('send', 'event', 'Directions page', 'Origin Search', `${self.originInputValue} was set as start position`);
			...
			// Google Analytics
			ga('send', 'event', 'Directions page', 'Destination Search', `${self.destinationInputValue} was set as destination`);
		}
	}
